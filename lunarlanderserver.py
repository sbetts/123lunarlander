...
startingPointList = [320, 280, 360, 400, 240, 200, 440, 160, 480, 120, 520, 80, 560]
...
def checkForCollision(lander1, lander2):
    if lander1.getRect().colliderect(lander2.getRect()) == True:
        if lander1.getStatus() == 'IN_PLAY' and lander2.getStatus() == 'IN_PLAY':
...
def playerLoses(lander, lossMessage):
    if lander.getStatus() != 'WINNER':
        lander.setLossMessage(lossMessage)


def destroyLander(landerLoc):
##    for x in range(36):
##        particleList.append(PropulsionParticle(landerList[landerLoc].getPosX(), landerList[landerLoc].getPosY(), 0, 2))
##        particleList.append(PropulsionParticle(landerList[landerLoc].getPosX() + 5, landerList[landerLoc].getPosY() - 3, 0, 2))
##        particleList.append(PropulsionParticle(landerList[landerLoc].getPosX() - 5, landerList[landerLoc].getPosY() - 3, 0, 2))
    landerList[landerLoc].setStatus('DESTROYED')
    landerList[landerLoc].setMoving(False)
...
def generateGameStateForClients(thisLanderList, thisParticleList):
    ...
    for x in thisLanderList:
        landerListForClients.append([thisLanderList[x].getAngle(), thisLanderList[x].getPosX(), thisLanderList[x].getPosY(),
                                     thisLanderList[x].getFuel(), thisLanderList[x].getPlayerID(), thisLanderList[x].getLossMessage(),
                                     thisLanderList[x].getVelocityY(), thisLanderList[x].getStatus()])
...
while True:
    ...
    for x in landerList:
        if checkForLanding(landerList[x], x):
            if landerList[x].getStatus() == 'IN_PLAY':
                landersToBeDestroyed.add(x)
